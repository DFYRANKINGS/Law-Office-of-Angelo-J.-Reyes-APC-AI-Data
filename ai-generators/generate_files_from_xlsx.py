...
def generate_footer_nav():
    ...
    <footer style="margin-top: 4rem; padding-top: 2rem; border-top: 1px solid #eee; color: #7f8c8d;">
        ...
        <div style="text-align:center;">
            <p>© {year} — Auto-generated from structured data. Last updated: {updated}</p>
        </div>
    </footer>
...
def generate_contact_page():
    ...
    Top 'Quick Contact' prefers contact_person (e.g., attorney name), then shows phone/email if present.
    """
    ...
    quick_label = ""     # lawyer/person if available; otherwise entity/location
    quick_phone = ""
    quick_email = ""
    ...
    for fname in sorted(os.listdir(locations_dir)):
        ...
        for loc in _normalize_records(data):
            ...
            entity_name = _first_nonempty(_alias_get(loc, "entity_name"), loc.get("location_name"), "Location")
            person = _first_nonempty(_alias_get(loc, "contact_person"))
            name_for_card = person or entity_name
            ...
            # Populate Quick Contact from the FIRST record only; prefer person then phone/email
            if not quick_label:
                quick_label = name_for_card or ""
            ...
            block += f"<h3>{escape_html(entity_name)}</h3><p>"
            if person:
    ...
    # Intro + ALWAYS show Quick Contact (prefers contact person)
    ...
    for file in os.listdir(reviews_dir):
        if file.endswith((".json", ".yaml", ".yml")):
            ...
            for rev in (rev_data if isinstance(rev_data, list) else [rev_data]):
                ...
                star_display = '★' * max(0, min(5, rating)) + '☆' * (5 - max(0, min(5, rating)))
                items.append(f"""
                <blockquote class="card" style="font-style: italic;">
                    ...
                    <footer style="margin-top: 1rem; font-style: normal;">
                        ...
                        {f'<br/><small>{escape_html(str(date))}</small>' if date else ''}
                    </footer>
...
def generate_about_page():
    ...
    # Locations / service areas and quick contact
    service_areas = set()
    phone = email = ""
...
if __name__ == "__main__":
    ...
    # Ensure GitHub Pages renders raw files correctly
